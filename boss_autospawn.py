...
- Mini-bosses: cada 6 horas (reemplaza al boss actual)
- Bosses: cada 3 días (reemplaza al boss actual)
    ...
    while not bot.is_closed():
        try:
            ...
            for guild in bot.guilds:
                ...
                # Verificar mini-boss (cada 6 horas)
                last_mini = LAST_SPAWN_TIMES["mini_boss"].get(guild_id)
                if last_mini is None or (current_time - last_mini).total_seconds() >= 21600:
                ...
                # Verificar boss normal (cada 3 días)
                last_boss = LAST_SPAWN_TIMES["boss"].get(guild_id)
                if last_boss is None or (current_time - last_boss).total_seconds() >= 259200:
